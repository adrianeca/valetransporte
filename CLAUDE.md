...
- `UNIDADES_SEM_VT = ['ONLINE']` — unidades que não têm VT nenhum (`isSemVT_()`): não aparecem no dropdown de unidades (`getAllowedUnidades_`), não recebem lembrete e não contam como pendentes (`getUnidadesAtivas_`). Também não devem constar em `DIRETORES_UNIDADE`.
