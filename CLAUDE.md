...
- **"+ Adicionar" identifica o funcionário pela POSIÇÃO no cadastro, nunca pela matrícula** (`value` = `indexOf(e)`): com duas matrículas placeholder ("-") iguais na unidade, escolher o segundo funcionário adicionava o primeiro de novo. Mesma correção feita no Horas e no VR. **Atenção**: no VT/VR a chave do lançamento continua sendo unidade+mês+ano+**matrícula** — dois funcionários com matrícula "-" na mesma unidade ainda colidiriam ao salvar, então `populateAddEmpDropdown` (que esconde matrícula já lançada) foi mantido de propósito: ele impede a colisão, ao custo de esconder o segundo "-". A saída definitiva é o DP preencher matrículas reais, ou portar a chave por nome que o Horas já usa.
